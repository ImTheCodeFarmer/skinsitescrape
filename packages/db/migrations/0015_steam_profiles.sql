-- Steam profile enrichment. For every player keyed by a Steam64 id
-- (Rustypot, RustEasy, Bandit.camp, and any Steam id the link job learns
-- of), the collector fetches what Steam shows publicly: persona, picture,
-- privacy, account age, bans, friends, and the profile's past names, which
-- Steam only exposes on the profile page. Everything is refreshed on a
-- schedule that follows how active the player is, under a global rate and
-- daily budget, so the job stays small however many players exist.
--
-- Freshness tiers (set by the collector, see core/steam.ts):
--   active in the last 7 days   refresh every 2 days, with aliases and friends
--   active in the last 30 days  refresh weekly, with aliases and friends
--   older                       refresh monthly, summary only
--   never fetched               first, newest activity first
...
CREATE TABLE IF NOT EXISTS steam_profiles (
  ...
  profile_url         text,
  vanity              text,
  visibility          text,            -- public | private | friends | unknown
  country             text,
  account_created_at  timestamptz,
  last_logoff_at      timestamptz,
  vac_banned          boolean,
  game_bans           integer,
  friends             text[],          -- Steam ids, capped at 500; null when private or not fetched
  friends_count       integer,
  friends_fetched_at  timestamptz,
  aliases_fetched_at  timestamptz,
...
CREATE INDEX IF NOT EXISTS steam_profiles_due_idx ON steam_profiles (next_fetch_at);
CREATE INDEX IF NOT EXISTS steam_profiles_avatar_idx ON steam_profiles (avatar_hash) WHERE avatar_hash IS NOT NULL;

-- Every name a Steam account has used, from the profile's alias history plus
-- each persona we see. norm_name matches the link job's normalisation.
CREATE TABLE IF NOT EXISTS steam_aliases (
...
CREATE INDEX IF NOT EXISTS steam_aliases_norm_idx ON steam_aliases (norm_name) WHERE norm_name IS NOT NULL;

-- The link job, now with Steam data:
--   * a Steam-keyed account whose site gives no usable picture takes its
--     Steam profile picture instead;
--   * a name match may use any past Steam alias of either account, not only
--     the name the site shows today (evidence.alias = true);
--   * two Steam-keyed accounts with different ids are never linked.
...
BEGIN
  ...
  WITH p AS (
    SELECT pl.site, pl.external_id,
           CASE WHEN pl.external_id ~ '^7656119[0-9]{10}$' THEN pl.external_id END AS steam_id,
           coalesce(steam_avatar_hash(pl.avatar), CASE WHEN pl.external_id ~ '^7656119[0-9]{10}$' THEN sp.avatar_hash END) AS ahash,
    ...
    FROM players pl
    LEFT JOIN steam_profiles sp ON sp.steam_id = pl.external_id
