-- refresh_player_links took 8.5 hours on 2026-09-23 (it had run in minutes)
-- and was cancelled; for all that time it held a lock on players, so the
-- 0018 migration could not add its column. Two changes, same results:
--
--   * The name check ran as a LATERAL subquery for every candidate pair,
--     scanning the pn CTE (no index) twice each time. It is now name_pairs,
--     computed once with one hash join and joined in.
--   * The avatar branch of pairs joined every pair of accounts sharing any
--     picture, including site default pictures shared by ~2,000 accounts
--     (~2.75M pairs, growing with the square of the owners), only for cand
--     to discard them: avatar evidence needs at most 6 owners and not one of
--     the two known defaults. That filter now applies in pairs. A pair
--     dropped here can still come in through its Steam id or name branch,
--     and cand scores it exactly as before.
--
-- The job also gets a max_runtime, so a run that goes wrong again is stopped
-- by TimescaleDB instead of holding players for hours.
...
BEGIN
  ...
  -- Every cross-site pair sharing a rare enough name, once, with the longest shared name and whether every match needed a past alias.
  -- (Was a LATERAL subquery per candidate pair that scanned pn twice each time: hours once shared avatars made millions of pairs.)
  name_pairs AS (
    SELECT pa.site AS site_a, pa.external_id AS player_a, pb.site AS site_b, pb.external_id AS player_b,
           max(length(pa.nname)) AS name_len, bool_and(pa.alias OR pb.alias) AS via_alias
  ...
  pairs AS (
    ...
    UNION
    -- Only avatars that can count as evidence (see cand): a default picture shared by thousands made millions of pairs that were all thrown away.
    ...
    JOIN ahash_owners ao ON ao.ahash = a.ahash AND ao.owners <= 6
    WHERE a.ahash NOT IN ('fef49e7fa7e1997310d705b2a6158ff8dc1cdfeb', 'b5bd56c1aa4644a474a2e4972be27ef9e82e517e')
    UNION
    SELECT site_a, player_a, site_b, player_b FROM name_pairs
  ),
  cand AS (
    SELECT x.site_a, x.player_a, x.site_b, x.player_b,
           ...
           nm.player_a IS NOT NULL AS name_exact,
    ...
    LEFT JOIN name_pairs nm ON nm.site_a = x.site_a AND nm.player_a = x.player_a AND nm.site_b = x.site_b AND nm.player_b = x.player_b
...
SELECT alter_job(job_id, max_runtime => INTERVAL '20 minutes')
FROM timescaledb_information.jobs WHERE proc_name = 'refresh_player_links';
