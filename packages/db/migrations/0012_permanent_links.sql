-- Permanent identity links. Once the hourly job scores a pair of accounts
-- as the same person (0.95 or higher: the same Steam id, or the same Steam
-- profile picture and the same name), the pair is recorded here keyed only
-- by the two sites' user ids, which never change. Later runs re-derive
-- player_links from whatever names and pictures the sites show at the time,
-- then merge these back in, so a rename or a new picture on one site can
-- lower a fresh score but never removes a link that was once certain.
--
-- `source` is 'auto' for the job; an admin can insert 'admin' rows the same
-- way (or delete a wrong one, which also removes it from player_links on
-- the next run).

CREATE TABLE IF NOT EXISTS player_links_confirmed (
