/**
 * Item coinflip on the main socket, channel "item-coinflip". Two players each
 * put in items worth about `betAmount`, the creator picks LOW or HIGH, and a
 * bot can be called in. `item-coinflip:new` and `item-coinflip:update` each
...
export function handleCoinflipGame(payload: unknown, receivedAt: Date, ctx: AdapterContext) {
  const g = payload as CgCoinflip;
  if (!g?.id || !Array.isArray(g.players)) return;
  const ids = new Map<number, string>();
  for (const p of g.players) {
    const id = p.user ? seen(p.user, receivedAt, ctx) : p.botId != null ? seenBot(p.botId, receivedAt, ctx) : null;
